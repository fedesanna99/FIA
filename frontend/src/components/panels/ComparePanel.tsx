/**
 * ComparePanel — confronto strutturale e di risultati fra due modelli (FASE 18).
 *
 * UX:
 *  - 2 select per modello A e B (model_id dal catalogo).
 *  - Toggle "include static diff" (esegue static su entrambi se non già salvata).
 *  - Mostra ModelDiff (nodi/elementi/loads/constraints added/removed/modified) +
 *    StaticResultsDiff (max delta nodo + tabella ElementForce delta).
 */
import { useState } from "react";
import { useQuery, useMutation } from "@tanstack/react-query";
import { GitCompare } from "lucide-react";
import { compareApi, type CompareResponse } from "../../api/compare";
...
export function ComparePanel() {
  ...
  const mut = useMutation({
    mutationFn: () => {
      if (!modelA || !modelB) throw new Error("Seleziona due modelli");
      if (modelA === modelB) throw new Error("Scegli due modelli diversi");
  ...
  return (
    <div className="p-3 space-y-3">
      <Card title="Confronto modelli" description="ModelDiff strutturale + delta risultati statici (FASE 18).">
        <div className="grid grid-cols-2 gap-2">
          ...
          <label className="flex items-center gap-2 text-xs pt-5">
            <input type="checkbox" checked={includeStatic}
