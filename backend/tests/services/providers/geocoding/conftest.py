"""Conftest condiviso per i test F4.2 (Open-Meteo + Nominatim geocoding)."""
...
def fast_limiter() -> RateLimiter:
    """RateLimiter con bucket open_meteo+nominatim a 10000 rps (no throttle)."""
    rl = RateLimiter()
    rl.register("open_meteo", rate_per_s=10000.0, capacity=10000.0)
    rl.register("nominatim", rate_per_s=10000.0, capacity=10000.0)
    return rl
...
def make_routed_transport(
    routes: dict[str, tuple[int, Any]],
) -> tuple[httpx.MockTransport, list[httpx.Request]]:
    """Mock transport con risposte diverse per path-suffix matching.

    routes: { "search": (200, body), "reverse": (200, body), ... }
    """
    received: list[httpx.Request] = []

    def handler(request: httpx.Request) -> httpx.Response:
        received.append(request)
        for suffix, (code, body) in routes.items():
        ...
        return httpx.Response(status_code=404, json={"error": "no route"})
